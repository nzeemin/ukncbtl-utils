 ...
 */
static int lzsa_get_literals_varlen_size_v2(const int nLength) {
 ...
 * @param nLength literals length
 */
static int lzsa_write_literals_varlen_v2(unsigned char *pOutData, int nOutOffset, const int nMaxOutDataSize, int *nCurNibbleOffset, int *nCurFreeNibbles, int nLength) {
   if (nLength >= LITERALS_RUN_LEN_V2) {
      if (nLength < (LITERALS_RUN_LEN_V2 + 15)) {
 ...
 * @return number of extra bits required
 */
static int lzsa_get_match_varlen_size_v2(const int nLength) {
 ...
 * @param nLength encoded match length (actual match length - MIN_MATCH_SIZE_V2)
 */
static int lzsa_write_match_varlen_v2(unsigned char *pOutData, int nOutOffset, const int nMaxOutDataSize, int *nCurNibbleOffset, int *nCurFreeNibbles, int nLength) {
   if (nLength >= MATCH_RUN_LEN_V2) {
      if (nLength < (MATCH_RUN_LEN_V2 + 15)) {
